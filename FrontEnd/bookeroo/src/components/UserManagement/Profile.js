        ...
        <Box width="100%">
            <Paper variant="outlined">
                <Box padding="3vh">
                    ...
                    <Typography variant="h6"> Account type: {userData.authorities.map((a) => a.authority).join(', ')} </Typography>
                    {isBuisness ? (
                            <Typography variant="h6"> ABN: {Object.values(userData.businessInfo)} </Typography>
                        ) : (
                            null
                        )}
                    </Box>       
